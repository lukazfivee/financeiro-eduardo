(() => {
  ...
  const serviceStatusLabel=s=>({orcamento:'Orçamento',aprovado:'Aprovado',em_execucao:'Em execução',em_andamento:'Em execução',aguardando_pagamento:'Aguardando pagamento',concluido:'Concluído',recebido:'Recebido',cancelado:'Cancelado'}[s]||'Em execução');
  let month=currentMonth();
  let mounted=false;
  let filters={type:'',category:'',status:'',q:''};
  ...
  function ensureUi(){
    ...
    let btn=document.querySelector('#navServices');
    if(!btn){
  ...
  async function renderServices(){
    ...
    try{
      ...
      <div class="kpi-grid services-kpis">
        ...
        <article class="kpi-card"><div class="kpi-top"><span class="kpi-icon result">↗</span><span>Lucro do mês</span></div><strong class="${dash.lucro_cents>=0?'positive':'negative'}">${money(dash.lucro_cents)}</strong><small>Margem ${Math.round(Number(dash.margem_percent||0))}%</small></article>
        <article class="kpi-card balance-card"><div class="kpi-top"><span class="kpi-icon wallet">R$</span><span>Resultado acumulado</span></div><strong>${money(dash.resultado_total_cents)}</strong><small>${dash.servicos_em_andamento||0} em execução • ${dash.clientes} clientes</small></article>
      </div>
      ...
      document.querySelector('#serviceFilterSearch').oninput=e=>{filters.q=e.target.value;clearTimeout(window.__serviceSearch);window.__serviceSearch=setTimeout(renderServices,250);};
      ...
      document.querySelectorAll('[data-service-detail]').forEach(b=>b.onclick=()=>openServiceDetails(list.items.find(x=>x.id===b.dataset.serviceDetail)));
  ...
  function serviceSummary(items){
    ...
    return `<div class="service-summary-list">${items.map(x=>{const profit=Number(x.receitas_cents||0)-Number(x.custos_cents||0);const margin=Number(x.receitas_cents||0)?Math.round(profit/Number(x.receitas_cents)*100):0;const receivable=Math.max(0,Number(x.contratado_cents||0)-Number(x.recebido_cents||0));return `<div class="service-summary-row"><div><strong>${esc(x.service_name)}</strong><small>${esc(x.client_name||'Cliente não informado')} • ${serviceStatusLabel(x.service_status)}</small></div><div class="service-values"><span>Contratado ${money(x.contratado_cents)}</span><span>Recebido ${money(x.recebido_cents)}</span><span>A receber ${money(receivable)}</span><span>Custo ${money(x.custo_realizado_cents)}</span><span>Margem ${margin}%</span><strong class="${profit>=0?'positive':'negative'}">${money(profit)}</strong></div></div>`;}).join('')}</div>`;
  }
  ...
  function openServiceModal(tx=null){
    ...
    document.querySelector('select[name="service_status"]').value=tx?.service_status==='em_andamento'?'em_execucao':tx?.service_status||'em_execucao';
  ...
  function openServiceDetails(tx){
    if(!tx)return;
    const receivable=Math.max(0,Number(tx.contracted_amount_cents||0)-Number(tx.received_amount_cents||0));
    const movement=Number(tx.amount_cents||0);
    document.body.insertAdjacentHTML('beforeend',`<div id="serviceDetailModal" class="modal"><div class="modal-card"><div class="modal-head"><div><span class="eyebrow">DETALHES DO SERVIÇO</span><h2>${esc(tx.service_name)}</h2><p>${esc(tx.client_name||'Sem cliente')}</p></div><button class="modal-close" id="closeServiceDetail">×</button></div><div class="detail-grid"><span>Status serviço</span><strong>${serviceStatusLabel(tx.service_status)}</strong><span>Status pagamento</span><strong>${statusLabel(tx.payment_status)}</strong><span>Descrição</span><strong>${esc(tx.description)}</strong><span>Data</span><strong>${esc(tx.transaction_date.split('-').reverse().join('/'))}</strong><span>Valor contratado</span><strong>${money(tx.contracted_amount_cents)}</strong><span>Valor recebido</span><strong>${money(tx.received_amount_cents)}</strong><span>A receber</span><strong>${money(receivable)}</strong><span>Custo previsto</span><strong>${money(tx.expected_cost_cents)}</strong><span>Custo realizado</span><strong>${money(tx.actual_cost_cents)}</strong><span>Movimentação</span><strong class="${tx.type==='entrada'?'positive':'negative'}">${tx.type==='entrada'?'+':'-'} ${money(movement)}</strong></div><div class="modal-actions full"><button class="btn secondary" id="editServiceFromDetail">Editar</button></div></div></div>`);
